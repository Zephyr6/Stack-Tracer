...
// Generated AddToManagers
		...
		public static void LoadStaticContent (string contentManagerName)
		{
			...
			if (IsStaticContentLoaded == false)
			{
				...
				if (!FlatRedBallServices.IsLoaded<Scene>(@"content/entities/textarial/scenefile.scnx", ContentManagerName))
				{
					registerUnload = true;
				}
				SceneFile = FlatRedBallServices.Load<Scene>(@"content/entities/textarial/scenefile.scnx", ContentManagerName);
		...
		public virtual bool HasCursorOver (FlatRedBall.Gui.Cursor cursor)
		{
			...
			if (cursor.IsOn3D(TextInstance, LayerProvidedByContainer))
			{
				return true;
			}
			return false;
